/**
 * Note that the original preact-signals source file has some mangling done.
 * So this is an unmangled version (to prevent errors).
 */
import { options, Component } from "../index";
import { useRef, useMemo, useEffect } from "../hooks";
import { signal, computed, batch, effect, Signal, } from "../signals-core";
...
hook("_diff" /* OptionsTypes.DIFF */, (old, vnode) => {
...
hook("_render" /* OptionsTypes.RENDER */, (old, vnode) => {
    setCurrentUpdater();
    let updater;
...
hook("_catchError" /* OptionsTypes.CATCH_ERROR */, (old, error, vnode, oldVNode) => {
    setCurrentUpdater();
...
hook("_hook" /* OptionsTypes.HOOK */, (old, component, index, type) => {
    if (type < 3)
